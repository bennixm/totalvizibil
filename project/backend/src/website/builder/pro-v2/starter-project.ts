...
export const STARTER_FILES: Record<string, string> = {
  ...
  // A real, pre-resolved lockfile for the exact dependency set above —
  // generated once (`npm install` against this same package.json) rather
  // than left for every fresh sandbox to resolve from scratch. Without it,
  // `npm install` has to hit the registry for version-range resolution on
  // every single boot; with it, npm goes straight to the pinned tarball URLs.
  // Measured impact: ~17s of a ~24s boot was resolution+install — this is
  // the single biggest lever on "why does the preview take so long".
  'package-lock.json': `{
  "name": "pro-v2-site",
  "version": "0.0.0",
  "lockfileVersion": 3,
