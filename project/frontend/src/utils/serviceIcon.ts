/**
 * Pick an mdi icon for a service from its name. Mirror of the backend
 * `src/website/drafts/service-icons.ts` — keep the two in sync. Used to show an
 * icon in the studio's service list and as the renderer's fallback when the
 * generated content carries no `icon`.
 */
...
/** Icon for a "why choose us" bullet — mirror of the backend `pickFeatureIcon`. */
